...
class Fuzzer:
    ...
    def get_valid(self, num: int, min_len: int, max_err: int) -> list:
        ...
        for main_method in main_methods:
            ...
            f.close()
            itemlen = 0
            #split bytecode to multiple tlv's and parse to generate function
            tlv_list = self.parse_tlv(end)
            valid = list()
            for tlv in tlv_list:
                print(tlv)
                itemlen += 1
                ...
                while len(valid) < num:
                    if not os.path.exists(f"output_classes/test{self.num_created}"):
                        os.mkdir(f"output_classes/test{self.num_created}")
                    valid.append(
                        self.generate(min_len, max_err,start, tlv,  main_method["code_attr_length"], main_method["max_lcls"])
                    )
                    self.num_created += 1
                for i in range(len(valid)):
                    tmp = valid[i]
                ...
                print("Method Name:",main_method["name"])
                print("Iterating tlv ",itemlen," for method",main_method["name"])
            # if i == 5:
            #     break
        return valid
