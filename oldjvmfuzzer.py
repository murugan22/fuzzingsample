...
class Fuzzer:
    ...
    def execute_binary(self) -> str:
        ...
        if len(stderr) != 0:
            ...
            if "Invalid pc" in err[1] or "Control flow" in err[1]:
                return "Incomplete" #we need to add more bytes to current #0x11/0x03 -> add one more byte to this sequence
            elif "Bad instruction" in err[1]:
                if err[1].strip().split(": ")[-1] != "Bad instruction":
                    if int(err[1].strip().split(": ")[-1], 16) >= 0xC9:
                        return "Error" #0x03 corrcet and incomplete  0x03/0x11
    ...
    def main_finder(self, path: str) -> tuple:
        ...
        for i in range(mcnt):
            ...
            print("Method Name:",consts[name_idx - 1].string)
